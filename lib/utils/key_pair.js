...
class PublicKey extends enums_1.Assignable {
    ...
    static fromString(encodedKey) {
        ...
        if (parts.length === 1) {
            return new PublicKeyEd25519({ keyType: KeyType.ED25519, data: serialize_1.base_decode(parts[0]) });
        }
        else if (parts.length === 2) {
            if (str_to_key_type(parts[0]) === KeyType.ED25519) {
                return new PublicKeyEd25519({ keyType: KeyType.ED25519, data: serialize_1.base_decode(parts[1]) });
            }
            else if (str_to_key_type(parts[0]) === KeyType.FALCON512) {
                return new PublicKeyFalcon512({ keyType: KeyType.FALCON512, data: serialize_1.base_decode(parts[1]) });
            }
            else {
...
class PublicKeyEd25519 extends PublicKey {
    toString() {
        return `${key_type_to_str(KeyType.ED25519)}:${serialize_1.base_encode(this.data)}`;
    }
...
class PublicKeyFalcon512 extends PublicKey {
    toString() {
        return `${key_type_to_str(KeyType.FALCON512)}:${serialize_1.base_encode(this.data)}`;
    }
    verify(message, signature) {
...
class KeyPairFalcon512 extends KeyPair {
    ...
    constructor(secretKey) {
        super();
        const pubKey = crypto_falcon_js_1.falcon.pubKey(serialize_1.base_decode(secretKey));
    ...
    static fromRandom() {
        ...
        return new KeyPairFalcon512(serialize_1.base_encode(newKeyPair.privateKey));
    }
    sign(message) {
        const signature = crypto_falcon_js_1.falcon.signDetached(message, serialize_1.base_decode(this.secretKey));
        return { signature, publicKey: this.publicKey };
    }
    verify(message, signature) {
        return this.publicKey.verify(message, signature);
