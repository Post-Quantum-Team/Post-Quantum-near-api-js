...
async function fetchJson(connectionInfoOrUrl, json) {
    ...
    const response = await (0, exponential_backoff_1.default)(START_WAIT_TIME_MS, RETRY_NUMBER, BACKOFF_MULTIPLIER, async () => {
        try {
            ...
            if (!response.ok) {
                ...
                throw (0, http_errors_1.default)(response.status, await response.text());
            }
            return response;
        }
        catch (error) {
