...
const getAccount2FA = async (account, keyMapping = ({ public_key: publicKey }) => ({ publicKey, kind: 'phone' })) => {
    ...
    account2fa.confirmKey = KeyPair.fromRandom('falcon512');
...
describe('deployMultisig key rotations (FALCON512)', () => {

    test('full access key if recovery method is "ledger" or "phrase", limited access key if "phone" (FALCON512)', async () => {
        const account = await testUtils.createAccount(nearjs);
        await account.addKey(KeyPair.fromRandom('falcon512').getPublicKey());
        await account.addKey(KeyPair.fromRandom('falcon512').getPublicKey());
        const keys = await account.getAccessKeys();
...
describe('account2fa transactions (FALCON512)', () => {

    test('add app key before deployMultisig (FALCON512)', async() => {
        let account = await testUtils.createAccount(nearjs);
        const appPublicKey = KeyPair.fromRandom('falcon512').getPublicKey();
